...
CREATE TABLE IF NOT EXISTS `ref_arah_infaqsedeqah` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='tabel ini untuk menjadi referensi bagi pengurus jika ada jemaah setor infq/sedekah mau di arafhkan ke mana masjid, yatim piatu atau fakir miskin';
...
CREATE TABLE IF NOT EXISTS `tb_panitia_zis` (
  ...
  `set_ttd1` int(11) DEFAULT 0,
  `set_ttd2` int(11) DEFAULT 0,
...
-- Membuang data untuk tabel olah_zakat.tb_panitia_zis: ~0 rows (lebih kurang)
...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `set_fitrah_beras` double DEFAULT NULL,
  `set_fitrah_uang` double DEFAULT NULL,
...
-- Membuang data untuk tabel olah_zakat.users: ~0 rows (lebih kurang)
